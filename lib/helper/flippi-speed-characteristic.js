...
const catcher = require('../module/catcher');
...
class FlipPiSpeedCharacteristic extends FlipPiCharacteristic {
	...
	onWriteRequest(data, offset, withoutResponse, callback) {
		...
		const content = raw ? catcher(() => JSON.parse(raw)) : null;
		...
		if (content && this.authenticator.isAllowed(content.pin)) {
			this._mutex.delayed(this._delayRequest)
				...
				.then(() => callback(Characteristic.RESULT_SUCCESS))
				.catch(() => callback(Characteristic.RESULT_UNLIKELY_ERROR));
